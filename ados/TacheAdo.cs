...
namespace GTP.ados
{
    public class TacheAdo
    {
        public static void AjouterTache(Tache t)
        {
            ...
            string query = "INSERT INTO Tache(titre_tache,desc_tache,fk_id_jalon) VALUES(@param1,@param2,@param3)";
            using (SqlCommand cmd = new SqlCommand(query, sqlc))
            {
                ...
                cmd.Parameters.Add("@param3", SqlDbType.Int).Value = t.Jalon;
                cmd.ExecuteNonQuery();
            }
        ...
        public static List<Tache> VoirTaches()
        {
            ...
            using (SqlDataReader reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    ...
                    Jalon jalon = JalonAdo.VoirJalons().First(x => x.Id == reader.GetInt32(3));
                    jalon.Taches.Add(t);
                    t.Jalon = jalon;
                    taches.Add(t);
                }
        ...
        {
            SqlConnection sqlc = Ado.OpenConnexion();
            string query = "UPDATE Tache SET titre_tache = @param1, desc_tache = @param2,fk_id_jalon = @param3 WHERE id_tache = @param4;";
            using (SqlCommand cmd = new SqlCommand(query, sqlc))
            {
                cmd.Parameters.Add("@param1", SqlDbType.VarChar).Value = t.Titre;
                cmd.Parameters.Add("@param2", SqlDbType.VarChar).Value = t.Description;
